...
class Test01:
    ...
    def send_management(self):
        ...
        try:
            try:
                ...
                self.first1.openpage.write_error_mysql(count, RESOURCE_ID_name1, report_name, report_name, '操作输出', 0,
                                                       "功能正常")
            except Exception as e:
                S_txt = '{}__进入iframe层:'.format(report_name) + str(e)
                self.first1.openpage.write_error_excel(S_txt)
                self.first1.openpage.write_error_mysql(count, RESOURCE_ID_name1, report_name, '进入iframe层', '操作输出', 1,
                                                       S_txt)
            ...
            # self.first1.openpage.cancel_button(count, RESOURCE_ID, report_name)
            # self.first1.openpage.save_button(count, RESOURCE_ID, report_name)
            # 任务启动
    ...
    def send_message_task(self):
        ...
        try:
            try:
                ...
                self.first1.openpage.write_error_mysql(count, RESOURCE_ID, report_name, '进入iframe层', '操作输出', 0, "功能正常")
            except Exception as e:
                S_txt = '{}__进入iframe层:'.format(report_name) + str(e)
                self.first1.openpage.write_error_excel(S_txt)
    ...
    def four_report_messages(self):
        ...
        try:
            ...
            self.first1.openpage.write_error_excel(S_txt)
            self.first1.openpage.write_error_mysql(count, RESOURCE_main_name, report_name, '进入iframe层', '操作输出', 1,
                                                   S_txt)
        count += 1
